...
def merge_patient_with_metadata(patients, dataset='acdc', path_to_df_meta=None):
    dataset = dataset.lower()

    known_paths = {'acdc': '/mnt/sds/sd20i001/sven/data/acdc/02_imported_4D_unfiltered/df_kfold.csv',
                   'tof': '/mnt/sds/sd20i001/sven/data/tof/02_imported_4D_unfiltered/SAx_3D_dicomTags_phase_outcome_meta_bsa'}

    if path_to_df_meta is None:
        assert dataset in known_paths.keys(), 'unknown dataset {} in function "merge_patient_with_metadata'.format(dataset)
        path_to_df_meta = known_paths[dataset]
    ...
    if dataset == 'acdc':
        ...
        merged_temp['target'] = merged_temp['pathology']

    elif dataset == 'tof':
        patients_df = pd.DataFrame(patients, columns=['pat'])
        patients_df['patient'] = patients_df['pat'].map(lambda x: x.split('_')[1].split('-')[1].upper())
        patients_df['fold'] = patients_df['pat'].map(lambda x: x.split('_')[0])

        df_meta = pd.read_csv(path_to_df_meta)
        df_meta = df_meta.loc[:, ~df_meta.columns.str.contains('^Unnamed')]
        df_meta['patient'] = df_meta['patient'].str.upper()
        df_meta = df_meta[['patient', 'Outcome y/n']]
        temp = df_meta.drop_duplicates(ignore_index=True, subset='patient')
        merged_temp = pd.merge(left=patients_df, right=temp, how='outer', left_on='patient', right_on='patient')
        merged_temp['target'] = merged_temp['Outcome y/n']

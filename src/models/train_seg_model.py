def train_fold(config, in_memory=True):
    ...
    from src.utils.SegKerasCallbacks import get_callbacks
    ...
    try:
        ...
        from src.models.predict_seg_model import pred_fold
        pred_fold(config)
        # exp_path = config.get('EXP_PATH')
        # evaluate(exp_path)
...
def main(args=None):
    ...
    if args.cfg:
        ...
        if args.data:  # if we specified a different data path (training from workspace or local node disk)
            ...
            config['DATA_PATH_ORIG'] = os.path.join(args.data, 'sax')
    ...
    from src.models.evaluate_seg import evaluate_cv
